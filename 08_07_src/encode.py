...
def get_train_data(aug_model, augments, tokenizer, args):
    ...
    for aug in augments:
        # 1. 문서 리스트를 하나의 긴 문자열로 병합 (줄바꿈으로 연결)
        psg = "\n".join(aug["passages"])
        
        # 2. 재작성된 문서 리스트를 하나의 긴 문자열로 병합
        rew = "\n".join(aug[f"{aug_model}_rewrite"])
        
        # 3. QA 리스트 가져오기 (메인 질문 포함 총 10개)
        qas = aug[f"{aug_model}_qa"]
        
        # QA 개수의 절반 계산 (10개면 5개)
        qpa_cnt = (len(qas) + 1) // 2
        
        for qid, qa in enumerate(qas):
            # Key Error 방지: full_answer가 없는 데이터(메인 질문)를 대비해 안전하게 가져오기
            target_answer = qa.get("full_answer", qa["answer"]) if args.with_cot else qa["answer"]
            
            if qid < qpa_cnt:
                # 합쳐진 전체 문서(psg)와 전체 재작성 문서(rew)를 각각 문맥으로 주고 학습
                for ppp in [psg, rew]:
                    prompt_ids.append(get_prompt(tokenizer, qa["question"], 
                                                    [ppp], 
                                                    target_answer, 
    ...
    for filename, fulldata in data_list:
        ...
        for did, data in tqdm(enumerate(fulldata), total=len(fulldata)):
            ...
            #     save_path = os.path.join(output_dir, f"data_{did}", f"passage_{pid}")
                ...
                #             init_adapter_path, save_path)
                model = train(None, [data], args, model, tokenizer, 
                              init_adapter_path, save_path)
